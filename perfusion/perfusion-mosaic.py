...
parser.add_argument("inputFiles", nARG='+', help="BF, BV, MTT, TTP (in this order)")
...
parser.add_argument("--cbf-range", '-cbf', help="CBF range in format min-max")
parser.add_argument("--cbv-range", '-cbv', help="CBV range in format min-max")
parser.add_argument("--mtt-range", '-mtt', help="MTT range in format min-max")
parser.add_argument("--ttp-range", '-ttp', help="TTP range in format min-max")

parser.add_argument("--ttp-range-file", help="TTP file without masking to determine TTP range")

parser.add_argument("--aif", nARG='+', type=int, help='x y z')


parser.add_argument("--layout", help="Image grid layout in format rows,column. Options are (1,4), (4,1) and (2,2). Default is (2,2)." )

parser.add_argument("--frames", nARG='+', type=int)
...
if (ARG.cbf_range):
    cbf_range=(list(map(float, ARG.cbf_range.split('-'))))
if (ARG.cbv_range):
    cbv_range=(list(map(float, ARG.cbv_range.split('-'))))
if (ARG.mtt_range):
    mtt_range=(list(map(float, ARG.mtt_range.split('-'))))
if (ARG.ttp_range_file):
    if (not ARG.aif):
        ...
        ttp_range.append(float(a)-10)
        ttp_range.append(float(a)+20)
if (ARG.ttp_range):
    ttp_range=(list(map(float, ARG.ttp_range.split('-'))))
#Set name prefix
name=""
if (ARG.name):
    name=ARG.name
...
if ARG.siemens:
    cmap = colors.ListedColormap(siemens_cb_cm)
elif ARG.asist:
...
#if ARG.mask_file and (not ARG.lower_percentile or not ARG.upper_percentile):
...
for f in frames:
    ...
    for ax, im in zip(grid, ARG.inputFiles):
        ...
        # p1=np.percentile(img, 100)

        # if os.path.splitext(os.path.basename(im))[0][-1] == 'A':
        #     p0=np.percentile(img, 1.5) #based on brain data
        #     p1=np.percentile(img, 98.5)
        # elif os.path.splitext(os.path.basename(im))[0][-1] == 'B':
        #     p0 = np.percentile(img, 0.5) #based on brain and liver data
        #     p1 = np.percentile(img, 99.5)

        # if (ARG.lower_percentile):
        p0 = np.percentile(img, ARG.lower_percentile)
        p1 = np.percentile(img, 100-ARG.lower_percentile)

        units_label = ''

        if 'BF' in im:
            if ARG.cbv_range:
                print('Using specified BF range')
                p0=cbf_range[0]
                p1=cbf_range[1]
            units_label = r'$\frac{mL}{100g \cdot min}$'
        if 'BV' in im:
            if ARG.cbv_range:
                print('Using specified BV range')
                p0=cbv_range[0]
                p1=cbv_range[1]
            units_label = r'$\frac{mL}{100g}$'
        if 'MTT' in im:
            if ARG.mtt_range:
                print('Using specified MTT range')
                p0=mtt_range[0]
                p1=mtt_range[1]
            units_label = r'$s$'
            if ARG.siemens:
                cmap = colors.ListedColormap(siemens_tt_cm)
        if 'TTP' in im:
            if ARG.ttp_range or ARG.ttp_range_file:
                print('Using specified TTP range')
                p0=ttp_range[0]
                p1=ttp_range[1]
            units_label = r'$s$'
            if ARG.siemens:
        ...
        img=ax.imshow(img, cmap=cmap, vmin=p0, vmax=p1)

        ax.axes.get_xaxis().set_visible(False)
        ax.axes.get_yaxis().set_visible(False)
        #bname=''
        bname = os.path.splitext(os.path.basename(im))[0][:3]
        #if os.path.splitext(os.path.basename(im))[0][-1] == 'A' or os.path.splitext(os.path.basename(im))[0][-1]=='B':
        #    bname = os.path.splitext(os.path.basename(im))[0][:-1]
        #else:
        ...
        ax.text(1,40, units_label, verticalalignment='top', horizontalalignment='left', color='white', fontsize=4)

        cbaxes = inset_axes(ax, width="3%", height="30%", loc=1)
        cb = fig.colorbar(img, cax=cbaxes, orientation='vertical', ticks=[math.ceil(p0), math.floor(p1)], )
        cb.ax.tick_params(labelsize=6, size=0, pad=0.1, labelcolor='white', labelleft='on', labelright='off')
        # cb.ax.set_xlabel(units_label, fontsize=4.2)
        # cb.ax.xaxis.label.set_color('white')
        # if len(units_label)>20:
